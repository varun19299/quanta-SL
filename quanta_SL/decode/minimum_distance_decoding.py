...
from quanta_SL.encode import binary_message, metaclass
from quanta_SL.utils.plotting import save_plot
from quanta_SL.utils.timer import CatchTimer
...
def faiss_flat_minimum_distance(x, y, index: NNDescent = None):
    """
...
def cpu_minimum_distance(x: NDArray[int], y: NDArray[int], gt_indices: NDArray[int]):
    ...
    # FAISS index
    with CatchTimer() as t:
        faiss_index = faiss.IndexBinaryFlat(y.shape[1] * 8)
        faiss_index.add(y)

    print(f"Index preparation time (FAISS Flat) {t.elapsed_time}")

    # FAISS
    with CatchTimer() as t:
        indices_faiss = faiss_flat_minimum_distance(x, y, index=faiss_index)

    timing_dict["FAISS"] = t.elapsed_time
    print(f"Query time (FAISS) {t.elapsed_time}")
    ...
    assert np.array_equal(indices_binary, indices_faiss)
...
if __name__ == "__main__":
    ...
    x, y, gt_indices = bch_dataset_query_points(
        bch_tuple, num_bits, query_repeat, verbose=True
    )
